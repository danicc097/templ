...
type goTemplElementExpressionParser struct{}

func (p goTemplElementExpressionParser) Parse(pi *parse.Input) (n Node, ok bool, err error) {
	// Check the prefix first.
	if _, ok, err = parse.Rune('@').Parse(pi); err != nil || !ok {
		return
	}

	r := TemplElementExpression{GoTempl: true}
	// Parse the Go expression.
	if r.Expression, err = parseGo("gotempl element", pi, goexpression.TemplExpression); err != nil {
		return r, false, err
	}
	...
	// Node contents.
	np := newGoTemplateNodeParser(closeBraceWithOptionalPadding, "gotempl  element closing brace")
	var nodes Nodes
...
var goTemplElementExpression goTemplElementExpressionParser
